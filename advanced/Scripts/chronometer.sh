...
gravity="${piholeVarDir}/gravity.list"
...
function CalcBlockedDomains(){
	...
	if [ -e "$gravity" ]; then
		...
		if [[ -n $piholeIPv6 ]];then
			#We are IPV6
			blockedDomainsTotal=$(wc -l "${piholeVarDir}/gravity.list" | awk '{print $1/2}')
		else
			#We are IPV4
			blockedDomainsTotal=$(wc -l "${piholeVarDir}/gravity.list" | awk '{print $1}')
		fi
	else
...
function normalChrono(){
	...
	do
		...
		#tail -f /var/log/pihole.log | awk '/\/etc\/pihole\/gravity.list/ {if ($7 != "address" && $7 != "name" && $7 != "${piholeVarDir}/gravity.list") print $7; else;}'
